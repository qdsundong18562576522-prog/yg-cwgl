...
import { Table, Button, Modal, Form, Select, DatePicker, Space, message, Card, Upload, Row, Col, Popconfirm } from 'antd';
    ...
    <div>
      <Card title="银行流水" extra={<Button type="primary" icon={<UploadOutlined />} onClick={() => setImportOpen(true)}>导入流水</Button>}>
        <Row gutter={16} style={{ marginBottom: 16 }}>
          ...
          <Col span={6}><Select placeholder="匹配状态" allowClear style={{ width: '100%' }} onChange={(v) => setParams(p => ({ ...p, matchStatus: v }))} options={[{ value: 'UNMATCHED', label: '未匹配' }, { value: 'AUTO_MATCHED', label: '自动匹配' }, { value: 'MANUALLY_MATCHED', label: '手工匹配' }]} /></Col>
